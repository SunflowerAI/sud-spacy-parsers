...
| `docs/generic-parser-v3.md` | the lexical channel returns, keyed by LEMMA and fillable at deployment from an ENGLISH GLOSS — so a language with no fastText can still use it | type coverage said 28.5 % where token coverage said 90.2 %, and hid four key folds; one of them was worth 55 points of Arabic |
