...
53 / 58), all of them `udep` → `mod` (392) or `udep` → `comp:obl` (32); **DEPREL is the only column
touched**. This is the extended relabel being rebuilt on the tokenisation the released arm now uses:
`ko_sud_gsd` switched from mecab morphemes to eojeol words, and the old morpheme-based case-particle
result (`comp:obl` F 0.169 → 0.386) depended on relations sitting on the particle, which eojeol
tokens fuse into the noun. So the relabel has to be redone at the coarser granularity, and this is
that work in progress.

**2. `relabel_cache_ext_ko.jsonl`, +138 entries** — the LLM decisions backing it (112 `modifier`,
26 `complement`). The keys reference `ko_gsd-sud-train.conllu`, i.e. the original treebank, which
confirms (1). This file is TRACKED, so `git checkout` on it discards the decisions and they would
have to be re-queried against Ollama.

**3. `configs/config_{fa,ja,lzh}_morph.cfg`** — regenerated by `retrain_udep_ruled.sh`:
`init_tok2vec` nulled and `source` repointed (lzh `training_lzh_both_ext/model-seg` →
`training_lzh_ext/model-best`). Deterministically regenerable, unlike (1) and (2).

None of this reached the v0.1.0 wheels: the shipped `ko_sud_gsd` is the eojeol arm trained BEFORE
this relabel, so its `comp:obl` F 38.6 does not yet include it.
