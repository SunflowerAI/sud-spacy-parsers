...
$PY -m spacy train configs/config_sa.cfg --output training_sa_csl_rev2/ --code $CODE \
  --paths.train $TRAIN --paths.dev $DEV 2>&1 | tee train_sa_csl_rev2.log

echo "=== [2/4] morphologizer (frozen base + dedicated encoder)"
$PY -m spacy train configs/config_sa_morph.cfg --output training_sa_morph2/ --code $CODE \
  --paths.train $TRAIN --paths.dev $DEV \
  --components.tok2vec.source training_sa_csl_rev2/model-best \
  --components.tagger.source  training_sa_csl_rev2/model-best \
  --components.parser.source  training_sa_csl_rev2/model-best 2>&1 | tee train_sa_morph2.log

echo "=== [3/4] lemmatizer (frozen base+morph, annotating_components=[morphologizer])"
$PY -m spacy train configs/config_sa_lemma.cfg --output training_sa_lemma2/ --code $CODE \
  --paths.train $TRAIN --paths.dev $DEV \
  --components.tok2vec.source       training_sa_morph2/model-best \
  --components.tagger.source        training_sa_morph2/model-best \
  --components.parser.source        training_sa_morph2/model-best \
  --components.morphologizer.source training_sa_morph2/model-best 2>&1 | tee train_sa_lemma2.log
