#!/usr/bin/env bash
# Retrain the yue chain on the subtype-preserving relabel (108 tokens: mod -> mod@tmod).
# The parser has to LEARN the label before it can ever produce it, so the small delta still needs
# the full chain: seg -> morph -> lemma -> SUD, then the xpos graft the released arm ships.
set -uo pipefail
cd "$(dirname "$0")/.."
PY=.venv/bin/python
MAX=${MAX_TRAINS:-1}
...
run train_yue_seg.log    configs/config_yue_seg.cfg    --output training_yue_seg/    --code scripts/seg_code.py \
