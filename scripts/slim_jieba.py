...
⚠ Do NOT prune `dict.txt` (4.8 MB of the surviving 7 MB). It IS the feature: `CLAUDE.md` records
jieba's traditional-vs-`t2s` gap as "entirely vocabulary", and the channel is worth +4.42 token F.
The corpus lexicon already covers corpus vocabulary; jieba's job is the words outside it.
...
def slim(site_packages: pathlib.Path, check: bool = False) -> int:
...
def main():
    ...
    sys.exit(slim(args.site_packages, args.check))
